...
mod tools {
    ...
    fn resolve(cwd: &Path, rel: &str) -> anyhow::Result<PathBuf> {
        // Cheap path-traversal guard: refuse anything that resolves
        // outside `cwd`. The model can still escape via `run_bash`,
        // but the file tools shouldn't be a free pass.
    ...
    pub fn list_files(cwd: &Path, rel: &str) -> anyhow::Result<String> {
        ...
        for entry in entries.flatten() {
            ...
            // Skip the usual noise without making this configurable —
            // the model can fall back to `run_bash` if it really
            // needs to peek inside `target/`.
    ...
    pub async fn run_bash(cwd: &Path, command: &str) -> anyhow::Result<String> {
        let output = tokio::process::Command::new("/bin/bash")
            ...
            .output()
            .await?;
        // Cap each stream so a runaway log doesn't blow the prompt
