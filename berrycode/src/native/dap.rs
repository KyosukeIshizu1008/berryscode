...
impl DapClient {
    ...
    pub async fn launch_adapter(&mut self, adapter_command: &str, args: &[&str]) -> Result<()> {
        ...
        let mut process = cmd.spawn()
            .context(format!("Failed to start debug adapter: {}", adapter_command))?;
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let content_length: Option<usize> = if line_buffer.starts_with("Content-Length:") {
                    line_buffer.trim_start_matches("Content-Length:").trim().parse().ok()
                } else {
                    None
                };

                if content_length.is_none() { continue; }

                // Skip to empty line
                loop {
                    line_buffer.clear();
                    if reader.read_line(&mut line_buffer).is_err() || line_buffer.trim().is_empty() { break; }
                }
                        ...
                        if let Ok(text) = String::from_utf8(content) {
                            if let Ok(value) = serde_json::from_str::<Value>(&text) {
                                ...
                                match msg_type {
                                    ...
                                    "event" => {
                                        let event_name = value.get("event").and_then(|e| e.as_str()).unwrap_or("");
                                        ...
                                        let event = match event_name {
                                            "stopped" => Some(DapEvent::Stopped {
                                                ...
                                                reason: body.get("reason").and_then(|r| r.as_str()).unwrap_or("unknown").to_string(),
                                            }),
    ...
    pub async fn set_breakpoints(&mut self, file_path: &str, lines: &[usize]) -> Result<Vec<DapBreakpoint>> {
        let breakpoints: Vec<Value> = lines.iter().map(|l| serde_json::json!({"line": l + 1})).collect();
    ...
    pub async fn get_stack_frames(&mut self, thread_id: u64) -> Result<Vec<DapStackFrame>> {
        ...
        if let Some(body) = response.get("body") {
            if let Some(sfs) = body.get("stackFrames").and_then(|s| s.as_array()) {
                for sf in sfs {
                    let source_path = sf.get("source")
                        .and_then(|s| s.get("path"))
                        .and_then(|p| p.as_str())
                        .map(String::from);
                    frames.push(DapStackFrame {
                        id: sf.get("id").and_then(|i| i.as_u64()).unwrap_or(0),
                        name: sf.get("name").and_then(|n| n.as_str()).unwrap_or("").to_string(),
            ...
            if let Some(vs) = body.get("variables").and_then(|v| v.as_array()) {
                for v in vs {
                    vars.push(DapVariable {
                        ...
                        variables_reference: v.get("variablesReference").and_then(|r| r.as_u64()).unwrap_or(0),
                    });
                }
            }
        }
    ...
    pub async fn disconnect(&mut self) -> Result<()> {
        let _ = self.send_request("disconnect", serde_json::json!({"terminateDebuggee": true})).await;
