...
// Cache loaded locale files once at startup.
static LANG_CACHE: Lazy<HashMap<&'static str, LangFile>> = Lazy::new(|| {
    let locales = ["ja_JP", "ko_KR"];

    locales
        .into_iter()
        .map(|locale| {
            let path = format!("assets/i18n/{locale}.toml");
...
pub fn t(lang: UiLanguage, en: &'static str) -> String {
    ...
    LANG_CACHE
        .get(lang.locale())
        .and_then(|file| file.entries.get(en))
        .cloned()
