...
async fn test_router_accuracy() {
    ...
    let cases: &[(&str, Role, &str)] = &[
        ...
        ("この関数にエラーハンドリングを追加して",        Role::Coder,      "エラーハンドリング追加"),
        ("パニックしているのを直して",                   Role::Coder,      "パニック修正"),
        ("新しいAPIエンドポイントを実装して",             Role::Coder,      "新機能実装"),
        ("リファクタリングして",                         Role::Coder,      "リファクタ"),
        ("Fix the compilation error in main.rs",        Role::Coder,      "compile error"),
        ("Implement a binary search function",          Role::Coder,      "implementation"),
        // architect
        ("システム設計のアドバイスをして",               Role::Architect,  "システム設計"),
        ("モジュール構成をどう設計すべきか",             Role::Architect,  "モジュール設計"),
        ("この機能のアーキテクチャを考えて",             Role::Architect,  "アーキテクチャ"),
        ("How should I structure this codebase?",       Role::Architect,  "structure"),
        // vision
        ("スクリーンショットのUIを確認して",             Role::Vision,     "UI screenshot"),
        ("このレイアウトのズレを指摘して",               Role::Vision,     "layout"),
        ("Check the UI screenshot for alignment issues",Role::Vision,     "screenshot EN"),
        // summarizer
        ("このコードを要約して",                         Role::Summarizer, "コード要約"),
        ("このエラーログを解説して",                     Role::Summarizer, "ログ解説"),
        ("このファイルが何をしているか教えて",            Role::Summarizer, "ファイル説明"),
        ("Explain what this module does",               Role::Summarizer, "explain EN"),
        ("Summarize this build output",                 Role::Summarizer, "summarize EN"),
        // summarizer — 設計+教えて context trap (should NOT route to architect)
        ("このプロジェクトの概要と設計教えて",           Role::Summarizer, "設計+教えて=summarizer"),
        ("このプロジェクトの設計を教えて",               Role::Summarizer, "設計を教えて=summarizer"),
        ("設計を説明して",                              Role::Summarizer, "設計を説明して=summarizer"),
        ("Explain the architecture of this project",   Role::Summarizer, "explain architecture EN"),
        // cli_git
        ...
        // reviewer
        ("セキュリティ脆弱性をチェックして",             Role::Reviewer,   "セキュリティ"),
        ("このunsafeブロックを監査して",                 Role::Reviewer,   "unsafe監査"),
        ("Audit this code for vulnerabilities",         Role::Reviewer,   "audit EN"),
        ("Check for memory safety issues",              Role::Reviewer,   "memory safety EN"),
        // doc_rag
...
async fn test_router_edge_cases() {
    ...
    let edge_cases: &[(&str, Role, &str)] = &[
        ...
        // Mixed intent
        ("バグを直してドキュメントも更新して",          Role::Coder,     "mixed: coder+doc"),
        ("設計を確認してコードも書いて",               Role::Architect, "mixed: architect+coder"),
        // Unusual phrasing
        ("gitのコミットメッセージをいい感じに",         Role::CliGit,   "unusual: commit"),
    ...
    let accuracy = correct as f64 / total as f64 * 100.0;
    println!("\n📊 Edge case accuracy: {}/{} ({:.1}%)", correct, total, accuracy);
...
async fn test_context_traps() {
    ...
    let cases: &[(&str, Role, &str)] = &[
        ...
        ("設計は変えなくていい、バグだけ直して",         Role::Coder,     "negation: 設計 but goal=fix"),
        ("設計はそのままでいいのでエラーを修正して",     Role::Coder,     "negation: 設計 but goal=error fix"),
        ("コミットはまだしないで。コードを直して",       Role::Coder,     "negation: commit but goal=fix"),
        ("コミットはまだしないで。まず脆弱性を探して",   Role::Reviewer,  "negation: commit, goal=security"),
        ("pushする前にセキュリティチェックして",         Role::Reviewer,  "before push: security"),
        // topic≠operation traps: topic word ≠ intended action
        ("GitのREADMEを書いて",                         Role::DocRag,    "topic=git but action=write docs"),
        ("tokioクレートの使い方を教えて",               Role::DocRag,    "crate usage → doc_rag not summarizer"),
        ("How do I use the tokio crate?",               Role::DocRag,    "crate usage EN → doc_rag"),
        // summarizer vs doc_rag boundary
        ("このコードが何をしているか教えて",             Role::Summarizer,"explain code → summarizer not doc"),
        ("このエラーの意味を教えて",                     Role::Summarizer,"explain error → summarizer"),
        // coder vs reviewer boundary
        ("セキュリティの脆弱性を直して",                 Role::Reviewer,  "fix+security → reviewer"),
        ("このunsafeを安全に書き直して",                 Role::Reviewer,  "rewrite unsafe → reviewer"),
        // coder vs cli_git boundary
        ("テストを書いてコミットして",                   Role::Coder,     "write tests first → coder"),
    ];
    ...
    for (msg, expected, desc) in cases {
        ...
        if ok { correct += 1; }
        let mark = if ok { "✅" } else { "❌" };
        println!("{} [{}] \"{}\" → {:?}  (expected {:?})", mark, desc, msg, got, expected);
    }
    let total = cases.len();
    let accuracy = correct as f64 / total as f64 * 100.0;
    println!("\n📊 Context-trap accuracy: {}/{} ({:.1}%)", correct, total, accuracy);
...
async fn test_sekkei_disambiguation() {
    ...
    let cases: &[(&str, Role, &str)] = &[
        ...
        ("このプロジェクトの概要と設計教えて",           Role::Summarizer, "概要と設計+教えて → summarizer"),
        ("このプロジェクトの設計を教えて",               Role::Summarizer, "設計+教えて → summarizer"),
        ("設計を教えて",                                Role::Summarizer, "設計を教えて → summarizer"),
        ("設計を説明して",                              Role::Summarizer, "設計を説明して → summarizer"),
        ("アーキテクチャを教えて",                      Role::Summarizer, "アーキテクチャ+教えて → summarizer"),
        ("Explain the architecture of this project",   Role::Summarizer, "explain architecture → summarizer"),
        ("Describe the current design",                Role::Summarizer, "describe design → summarizer"),
        // architect — して / 考えて / すべきか / 提案して dominates
        ("システム設計のアドバイスをして",               Role::Architect,  "設計アドバイス → architect"),
        ("どんな設計にすべきか",                        Role::Architect,  "設計すべきか → architect"),
        ("設計を考えて",                                Role::Architect,  "設計を考えて → architect"),
        ("設計を提案して",                              Role::Architect,  "設計を提案して → architect"),
        ("How should I design this module?",           Role::Architect,  "how to design → architect"),
    ];
    ...
    let total = cases.len();
    let accuracy = correct as f64 / total as f64 * 100.0;
    println!("\n📊 設計 disambiguation accuracy: {}/{} ({:.1}%)", correct, total, accuracy);
